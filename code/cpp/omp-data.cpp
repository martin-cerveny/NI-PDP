...
constexpr int ENOUGH_STATES_COEF = 500;
...
public:
    ...
    void solve(const int enoughStates) {
        ...
        while (!bfsQueue.empty() && bfsQueue.size() + work.size() < enoughStates) {
...
int main(int argc, char* argv[]) {
    ...
    if (solver.read()) {
        ...
        solver.solve(ENOUGH_STATES_COEF * numThreads);
