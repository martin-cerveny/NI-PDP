...
struct Solution {
    ...
    inline CellType& type(const int r, const int c) { return boardState[r][c].type; }
    inline CellType type(const int r, const int c) const { return boardState[r][c].type; }
    inline CellType& type(const Coordinates p) { return boardState[p.r][p.c].type; }
    inline CellType type(const Coordinates p) const { return boardState[p.r][p.c].type; }
};
...
public:
    ...
    inline CellType type(const int r, const int c) const { return solution.type(r, c); }
    inline CellType type(const Coordinates p) const { return solution.type(p); }
    inline void setType(const Coordinates p, const CellType t) { solution.type(p) = t; }
    inline void setCell(const int r, const int c, const CellType t, const int id) { solution.boardState[r][c] = {t, id}; }
};
...
public:
    ...
    void setDepthLimit(const int depth) {
        depthLimit = depth;
    }
        ...
        if (depth < depthLimit) {
            for (const auto& shape : ShapesUpperLeft) {
                if (canPutShape(current, shape, p.r, p.c, NOT_DECIDED)) {
                    ...
                    #pragma omp task shared(bestSolution, bestPriceShared) firstprivate(current)
...
int main(int argc, char* argv[]) {
    ...
    Solver solver;
    solver.setDepthLimit(std::max(2, static_cast<int>(std::log2(numThreads)) + 2));
    if (solver.read()) {
